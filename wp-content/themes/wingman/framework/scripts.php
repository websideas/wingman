...
if(is_admin()){
    if ( !function_exists( 'kt_admin_enqueue_scripts' ) ) {
        ...
        function kt_admin_enqueue_scripts(){
            ...
            wp_register_style( 'simple_line_icons', THEME_FONTS.'simple_line_icons/simple-line-icons.css');
            wp_register_style( 'framework-core', FW_CSS.'framework-core.css');
            wp_register_style( 'chosen', FW_LIBS.'chosen/chosen.min.css');


            wp_register_style( 'admin-style', FW_CSS.'theme-admin.css',array( 'elegant_font', 'kt-font-awesome', 'simple_line_icons', 'framework-core','chosen'));
...
if(!function_exists('kt_admin_custom_style')){
    /**
    ...
    function kt_admin_custom_style(){
        ...
        wp_register_style('kt_style_admin', FW_CSS.'login-admin.css');
        ...
        $ouput = '';
        if( $logo_admin['url'] ){
            $ouput .= 'h1 a { background-image:url('.$logo_admin['url'].') !important;background-size: auto auto !important;margin-bottom: 10px !important;width:auto!important;}';
        }
        if( $bg_admin['background-color'] || $bg_admin['background-image'] ){
            $ouput .= 'body.login{ background-image:url('.$bg_admin['background-image'].') !important;background-size: '.$bg_admin['background-size'].';background-repeat:'.$bg_admin['background-repeat'].';background-position:'.$bg_admin['background-position'].';background-color:'.$bg_admin['background-color'].';background-attachment:'.$bg_admin['background-attachment'].';}';
        }
